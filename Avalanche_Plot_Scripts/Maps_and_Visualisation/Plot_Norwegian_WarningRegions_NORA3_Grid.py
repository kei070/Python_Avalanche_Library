...
# -*- coding: utf-8 -*-
"""
Plot the Norwegian warning regions on the NORA3 grid and print their sizes.
"""
...
print(f"\nAverage region size Norway: {np.round(warn_amean)} km^2")
print(f"\nAverage A-region size Norway: {np.round(warn_a_amean)} km^2")
print(f"\nAverage B-region size Norway: {np.round(warn_b_amean)} km^2")
print(f"\nAverage region size northern Norway: {np.round(warn_north_amean)} km^2")
...
"""
shp_orth = shp.to_crs(crs=orth_crs.proj4_init)


#%% set the region code as an index
shp_orth.set_index("region", inplace=True, drop=False)


#%% calculate the area of the regions
shp_orth["area"] = shp_orth["geometry"].area * 1e-6


#%% extract the northern regions
shp_orth_north = shp_orth[(shp_orth.region < 3014) & (shp_orth.region > 3008)]
"""


#%% print the region sizes
# print(shp_orth["area"])
...
p00 = ax00.pcolormesh(lons_full, lats_full, warn, cmap=cmap, transform=ccrs.PlateCarree())
...
warn_a_info.plot(ax=ax00, legend=False, zorder=101, linewidth=1.8,
                 legend_kwds={"ncol":1, "loc":"upper left"}, facecolor="none",
                 edgecolor="black", cmap="Set1")
...
ax00.plot([x11, x12], [y11, y12], transform=ccrs.PlateCarree(), c="black")
ax00.plot([x21, x22], [y21, y22], transform=ccrs.PlateCarree(), c="black")

ax00.plot([x11, x21], [y11, y21], transform=ccrs.PlateCarree(), c="black")
ax00.plot([x12, x22], [y12, y22], transform=ccrs.PlateCarree(), c="black")

ax00.set_extent([4.9, 25.5, 57.75, 71.5])
