...
#%% prepare the data for the per-year per-region plot
data_uni_it, data_hist_it = disc_hist_data([ar_it[ar_it.win_year == yr]["danger_level"] for yr in
                                            np.unique(ar_it.win_year)], classes=None)
data_uni_ly, data_hist_ly = disc_hist_data([ar_ly[ar_ly.win_year == yr]["danger_level"] for yr in
                                            np.unique(ar_ly.win_year)], classes=None)
data_uni_nt, data_hist_nt = disc_hist_data([ar_nt[ar_nt.win_year == yr]["danger_level"] for yr in
                                            np.unique(ar_nt.win_year)], classes=None)
data_uni_tr, data_hist_tr = disc_hist_data([ar_tr[ar_tr.win_year == yr]["danger_level"] for yr in
                                            np.unique(ar_tr.win_year)], classes=None)
data_uni_st, data_hist_st = disc_hist_data([ar_st[ar_st.win_year == yr]["danger_level"] for yr in
                                            np.unique(ar_st.win_year)], classes=None)


#%% plot the avalanche danger per region and per year (one plot per region)
f_col = "none"
marker = "_"
lwd = 3
xlim = (0.85, 5.15)
...
for i in np.arange(len(data_uni_nt)):
    ax00.scatter(data_uni_nt[i], data_hist_nt[i], marker=marker, label=win_year[i], color=colors[i], linewidth=lwd)
    ax01.scatter(data_uni_ly[i], data_hist_ly[i], marker=marker, color=colors[i], linewidth=lwd)
    ax10.scatter(data_uni_tr[i], data_hist_tr[i], marker=marker, color=colors[i], linewidth=lwd)
    ax11.scatter(data_uni_st[i], data_hist_st[i], marker=marker, color=colors[i], linewidth=lwd)
    ax20.scatter(data_uni_it[i], data_hist_it[i], marker=marker, color=colors[i], linewidth=lwd)
...
xtxt, ytxt = 3, 120
h_align = "center"
ax00.text(xtxt, ytxt, s="Nord-Troms", horizontalalignment=h_align)
ax01.text(xtxt, ytxt, s="Lyngen", horizontalalignment=h_align)
ax10.text(xtxt, ytxt, s="Tromsø", horizontalalignment=h_align)
ax11.text(xtxt, ytxt, s="Sør-Troms", horizontalalignment=h_align)
ax20.text(xtxt, ytxt, s="Indre Troms", horizontalalignment=h_align)
...
# y_text = -70
# for x_text, s_text in ndlev_it.items():
...
#%% plot the avalanche danger per region and per year (one plot per year)
f_col = "none"
marker = "_"
lwd = 3
xlim = (0.85, 5.15)
...
axes = [fig.add_subplot(k) for k in np.arange(421, 429, 1)]

# axes[i].set_xlim(xlim)
# axes[i].set_ylim(ylim)

for i in np.arange(len(axes)):
    axes[i].scatter(data_uni_nt[i], data_hist_nt[i], marker=marker, color="black", linewidth=lwd)
    axes[i].scatter(data_uni_ly[i], data_hist_ly[i], marker=marker, color="blue", linewidth=lwd)
    axes[i].scatter(data_uni_tr[i], data_hist_tr[i], marker=marker, color="red", linewidth=lwd)
    axes[i].scatter(data_uni_st[i], data_hist_st[i], marker=marker, color="gray", linewidth=lwd)
    axes[i].scatter(data_uni_it[i], data_hist_it[i], marker=marker, color="orange", linewidth=lwd)
    axes[i].text(xtxt, ytxt, s=win_year[i], horizontalalignment=h_align)
    axes[i].axhline(y=0, c="black", linewidth=0.5)
    axes[i].set_xlim(xlim)
    axes[i].set_ylim(ylim)
...
axes[0].scatter([], [], color="black", marker=marker, label="Nord-Troms", linewidth=lwd)
axes[0].scatter([], [], color="blue", marker=marker, label="Lyngen", linewidth=lwd)
axes[0].scatter([], [], color="red", marker=marker, label="Tromsø", linewidth=lwd)
axes[0].scatter([], [], color="gray", marker=marker, label="Sør-Troms", linewidth=lwd)
axes[0].scatter([], [], color="orange", marker=marker, label="Indre Troms", linewidth=lwd)
...
data_te_list, data_te_hist_list = disc_hist_data([ar_nnor[ar_nnor.win_year.isin(split)]["danger_level"]],
                                                 classes=None)
split = [2017, 2018, 2019, 2020, 2022, 2024]
