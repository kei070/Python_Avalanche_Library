...
# -*- coding: utf-8 -*-
"""
Aggregate the EURO-CORDEX predictors across the elevation bands.
"""
...
reg_code = int(sys.argv[3])  # 3010
agg_type = "mean"
a_p = "all"


#%% set paths
data_path = f"/{path_par3}/IMPETUS/EURO-CORDEX/Avalanche_Region_Predictors/{scen}/{model}/"
...
for h_lo, h_hi in zip([0, 300, 600, 900], [300, 600, 900, 1200]):
    ...
    fn_in = f"Between{h_lo}_and_{h_hi}m/{regions[reg_code]}_{model}_Predictors_MultiCellMean_" + \
...
#%% store the features
result_feats.to_csv(data_path + f"{regions[reg_code]}_{model}_{scen}_ElevAgg_Predictors_MultiCellMean.csv")
