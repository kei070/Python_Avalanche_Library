...
from ava_functions.Lists_and_Dictionaries.Variable_Name_NORA3_NorCP import varn_norcp


#%% parser
parser = argparse.ArgumentParser(
                    description="""Calculate the predictors for the NorCP downscalings.""")
...
parser.add_argument('--scen', default="rcp45", choices=["historical", "evaluation", "rcp45", "rcp85"], type=str,
                    help='Scenario used in the downscaling.')
parser.add_argument('--period', default="MC", choices=["", "MC", "LC"], type=str,
...
#%% NorCP run directory
ncp_dir = f"{model.upper()}_{scen}{period}/"


#%% set the data path
data_path = f"/{path_par}/IMPETUS/NorCP/Avalanche_Region_Data//Between{h_low}_and_{h_hi}m/{ncp_dir}/"
data_path_sn = f"/{path_par}/IMPETUS/NorCP/Avalanche_Region_Data_seNorge/Between{h_low}_and_{h_hi}m/{ncp_dir}"
...
prec_pred = cprf.calc_prec_pred(data_path=data_path, reg_code=reg_code, h_low=h_low, h_hi=h_hi,
                                agg_type=agg_type, perc=perc, varn_dic=varn_norcp, unclean=unclean)
...
wdrift_pred = cprf.calc_wdrift_pred(reg_code=reg_code, prec_pred=prec_pred, wind_pred=wind_pred, h_low=h_low, h_hi=h_hi,
                                    unclean=unclean)
...
out_path = f"/{path_par}/IMPETUS/NorCP/Avalanche_Region_Predictors/{agg_str}/{ncp_dir}/Between{h_low}_and_{h_hi}m/"
# out_path = out_path + ncp_dir
os.makedirs(out_path, exist_ok=True)
df.to_csv(out_path + f"{region}_NorCP_Predictors_MultiCell{agg_str}_Between{h_low}_and_{h_hi}m.csv")
